@testset "Config placeholder parsing" begin
    mktempdir() do root
        ...
        @test config.subfolder_template == "yyyy_mmdd"
...
@testset "Config mutability" begin
    mktempdir() do root
        ...
        @test endswith(current_collection_path(config), joinpath("2024_0101", "phase_01"))
        ...
        @test_throws ArgumentError set_intermediate_template!(
            config,
            "phase_##";
            index_width = 3,
            now = now,
...
@testset "Path generation and directory lifecycle" begin
    mktempdir() do root
        ...
        path = current_collection_path(config)
        @test endswith(path, joinpath("2024_0525", "run_01"))
...
@testset "Date rollover" begin
    mktempdir() do root
        ...
        @test occursin("2024_0801", rollover_path)
    end
end
